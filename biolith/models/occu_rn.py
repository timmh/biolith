...
def occu_rn(
    ...
    # Occupancy process
    abundance = numpyro.deterministic("abundance", jnp.exp(jnp.tile(beta[0], (n_sites,)) + jnp.dot(beta[1:], site_covs) + w))
    ...
    with numpyro.plate('site', n_sites, dim=-1):
        ...
        with numpyro.plate('time', time_periods, dim=-2):
            ...
            # Detection process
            r_it = numpyro.deterministic(f'prob_detection', jax.nn.sigmoid(jnp.tile(alpha[0], (time_periods, n_sites)) + jnp.sum(alpha[1:, None, None] * obs_covs, axis=0)))
